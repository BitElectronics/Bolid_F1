...
#define LINE_TRESHOLD	  35	  // Ниво под което се смята че няма линия под сензора
#define SENSOR_TRESHOLD	15    // Филтриране на шумове от сензорите
...
#define TURN_SPEED_HIGH     MAX_SPEED + MAX_SPEED/6
#define TURN_SPEED_LOW      0


// Лява и дясна позиция, след които се изпуска линията
#define TURN_ERROR_LEFT    110
#define TURN_ERROR_RIGHT   590
...
void loop() {
  ...
  /*
    ...
    sensor_values[0] = analogRead(A7);
    sensor_values[1] = analogRead(A6);
    sensor_values[2] = analogRead(A5);
    sensor_values[3] = analogRead(A4);
    sensor_values[4] = analogRead(A3);
    sensor_values[5] = analogRead(A2);
  */
  Serial.print("Position ");
  Serial.print(position);
  sprintf(tmp_str, "  / %3d  %3d  %3d", sensor_values[0],  sensor_values[1],  sensor_values[2]);
  Serial.print(tmp_str);
  sprintf(tmp_str, "  %3d  %3d  %3d", sensor_values[3],  sensor_values[4],  sensor_values[5]);
...
unsigned int read_position(void) {
  ...
  if (!on_line) {
    ...
    if (on_line >2) {
      last_pos = 350;
      if ( ( sensor_values[0] > 80) && !sensor_values[SENSORS_NR-1])
        last_pos = 100;
     if  ( (sensor_values[SENSORS_NR-1] > 80) && !sensor_values[0] )
        last_pos = (SENSORS_NR * 100);
